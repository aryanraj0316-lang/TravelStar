...
// Default seed data for monsoon alerts
const SEED_ALERTS = [
  {
    severity: 'CRITICAL',
...
router.get('/', async (req, res) => {
  try {
    ...
    if (alerts.length === 0 && !seeded) {
      seeded = true;
      await prisma.alert.createMany({ data: SEED_ALERTS });
    ...
    console.warn('[Alerts] DB error, returning seed data:', err);
    const fallback = SEED_ALERTS.map((a, i) => ({ id: `alert-${i + 1}`, ...a, active: true, createdAt: new Date().toISOString() }));
    res.status(200).json({ status: 'success', data: fallback });
  }
});

// Create a new alert (admin post — visible to all users)
router.post('/', async (req, res) => {
  const { severity, title, category, location, time, desc, affectedRoute, precautions, image } = req.body;
  try {
    const alert = await prisma.alert.create({
      data: {
        severity: severity || 'ADVISORY',
        title: title || 'New Alert',
        category: category || 'FLOOD & RAIN',
        location: location || 'India',
        time: time || 'Just now',
        desc: desc || '',
        affectedRoute: affectedRoute || '',
        precautions: precautions || [],
        image: image || 'https://images.unsplash.com/photo-1518156677180-95a2893f3e9f?w=600&q=80',
      },
    });
